...
export async function askNagrikAI(userMessage, history = [], { ragContext = "", sources = [] } = {}) {
  const ragInstructions = ragContext
    ? `
IMPORTANT — Verified knowledge base:
- Base your answer primarily on the VERIFIED CONTEXT below.
- If the context covers the question, use it and mention the official scheme or service name.
- If the context does NOT cover the question, say you don't have verified details for that specific topic and suggest checking the official government portal. Do not invent scheme names, eligibility rules, or document lists.
- At the end, briefly mention which official source(s) the citizen should visit.

VERIFIED CONTEXT:
${ragContext}`
    : `
Note: No verified documents matched this query. Answer from general knowledge but clearly state when details may vary by state and recommend checking the official portal.`;
